...
class TraceALine:
    def __init__(self):
        ...
        # except:
        #     self.use_topic_not_service = True
        self.use_topic_not_service = True
        ...
        assert (
            len(tolerances) % 6 == 0
        ), "The number of tolerances should be a multiple of 6"
        for i in range(int(len(tolerances) / 6)):
            self.tolerances.append(
                Twist(
                    Vector3(
                        tolerances[i * 6], tolerances[i * 6 + 1], tolerances[i * 6 + 2]
                    ),
                    Vector3(
                        tolerances[i * 6 + 3],
                        tolerances[i * 6 + 4],
        ...
        while not rospy.is_shutdown():
            print("Start heart tracing in {} seconds".format(count_down))
            count_down -= 1
            if count_down == 0:
                break
        ...
        self.timer = rospy.Timer(rospy.Duration(0.003), self.timer_callback)
    ...
    def generate_trajectory(self):
        ...
        for i in range(num_points):
            ...
            for j in range(self.robot.num_chain):
                poses[j].position.y += y
                poses[j].position.z += z
